...
def lsh_mc(user_rank_matrix,user_style_matrix,P,Q,opts):
    ...
    for i in range(opts['split_number']):
        for j in range(opts['split_col_number']):
            sub_matrix[str(i) + '' + str(j)] = [(split_number_lsh[i, 0] + split_number_lsh[i, 1]) / 2,
                                                (split_col_number_lsh[j, 0] + split_col_number_lsh[j, 1]) / 2]
    ...
    for i in range(opts['split_number']):
        for j in range(opts['split_col_number']):
            ...
            middle = sub_matrix[str(i) + '' + str(j)]
            distance_matrix = np.zeros((R.shape))
            for p in range(R.shape[0]):
                for q in range(R.shape[1]):
                    d_1 = np.sqrt(
                        np.square((test_lsh_index[loc[i,2]+p, 1] - middle[0])) + np.square((test_col_index[loc_col[j,2]+q, 1] - middle[1])))
                    distance_matrix[p,q] = d_1

            nP,nQ = SGD.SGD_weight(R, P_lsh,Q_lsh,K,distance_matrix,opts['step'],opts['alpha'],opts['beta'],opts['tol'])
